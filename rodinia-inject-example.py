...
def compileInjectionPass():
	...
	# Compile to injection pass and pathfinder program
...
def runInjection(totalFiNumber):
	...
	for fiNumber in range(1, totalFiNumber+1):
		...
		# Use getLastError instead of pathfinder-gdb
		if useGdb == False:
			try:
			...
			# Get std output
			try:
				p = pexpect.spawn("pathfinder-gdb " +outputExeFile)
				p.timeout = 5000
				p.expect("pathfinder-gdb")
