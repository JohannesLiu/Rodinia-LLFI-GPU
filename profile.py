...
bitIndex = "" # 63 or ""	
#############################################################################
flagHeader = staticInstIndex + " CICC_MODIFY_OPT_MODULE=1 LD_PRELOAD=./libnvcc.so /usr/local/pathfinder/bin/nvcc -arch=sm_30 -rdc=true -dc -g -G -Xptxas -O0 -D BAMBOO_PROFILING"
...
def runProfile():
	...
	os.system("/usr/local/pathfinder/bin/nvcc -arch=sm_30 profiling_runtime.o " + linkList + " -o " + outputExeFile + " -O0 " + linkFlags)
